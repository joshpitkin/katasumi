    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8">
          {welcome && (
            <div className="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 text-green-800 dark:text-green-200 px-4 py-3 rounded">
              Your account is active! Please log in to get started.
            </div>
          )}

          {oauthError === 'no_account' && (
            <div className="mb-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 text-yellow-800 dark:text-yellow-200 px-4 py-3 rounded">
              No account found for that email.{' '}
              <Link href="/signup" className="font-medium underline">Sign up instead?</Link>
            </div>
          )}

          {oauthError === 'pending_payment' && (
            <div className="mb-6 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 text-yellow-800 dark:text-yellow-200 px-4 py-3 rounded">
              Your account is pending payment. Please complete checkout via{' '}
              <Link href="/signup" className="font-medium underline">sign up</Link>.
            </div>
          )}

          {oauthError && !['no_account', 'pending_payment'].includes(oauthError) && (
            <div className="mb-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-200 px-4 py-3 rounded">
              Social login failed. Please try again or use email and password below.
            </div>
          )}
